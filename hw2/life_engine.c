...
{
	return board.num_cols * row + col; // TODO: implement me
}
/********************************************************************
...
{   if(board.num_rows > row  && 0 <= row && board.num_cols > col && 0 <= col){
        return 0;
    }else{
	    return 1;
    }// TODO: implement me
...
{   
    if (is_in_range(board, row, col) == 0 && board.cells[get_index(board, row, col)] == 1){
        return 0;
    }else{
	    return 1;
    }// TODO: implement me
...
{   
    ...
    for (int i = -1; i < 2; i++){
        for (int j = -1; j <2; j++){
            if (is_alive(board,row+i,col+j) == 0){
                count++;
            }

        }               
    }
    if (is_alive(board,row,col) == 0){
        count--;
    }
...
{   for (int row = 0; row < current.num_rows; row++){
        for (int col = 0; col < current.num_cols; col++){
            if (is_alive(current,row,col) == 0) {
               
                if (count_live_nbrs(current, row, col) >= 4){
                    set_dead(next, row,col);
                }else if (count_live_nbrs(current, row, col) >= 2){
                    set_alive(next,row,col);
                }else{
                    set_dead(next,row,col);
                }
            }else{
                if (count_live_nbrs(current,row,col) == 3){
                    set_alive(next,row,col);
                }else{
                    set_dead(next,row,col);
                }
